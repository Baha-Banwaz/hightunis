    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          {items.map((t) => (
            <div key={t.id} className="bg-white border border-black/20 p-6 flex items-start gap-6">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => togglePublished(t.id, t.published)}
                  className={`w-8 h-8 flex items-center justify-center border border-black/20 transition-colors ${t.published ? "bg-black text-white" : "bg-black/10 text-black/30"}`}>
                  <Check size={14} />
                </button>
